...
def test_task_creation_is_persisted_and_created(client):
    ...
    with SessionLocal() as db:
        ...
        assert task is not None and task.status is TaskStatus.CREATED
...
def test_task_pagination_and_filters(client):
    ...
    assert len(client.get("/v1/tasks?status=CREATED", headers=auth_headers(token)).json()["items"]) == 2
